...
APP=kaffeine
BIN="$APP"
DEPENDENCES="gcc-libs glibc kconfig5 kconfigwidgets5 kcoreaddons5 kdbusaddons5 ki18n5 kio5 kwidgetsaddons5 kwindowsystem5 kxmlgui5 qt5-base solid5 v4l-utils vlc kio5-extras switcheroo-control breeze-icons hicolor-icon-theme qt5-tools"
...
cd $APPDIR
desktopfile=~/.local/share/applications/org.kde.kaffeine.desktop
iconfile=~/.local/share/pixmaps/$prg.svg

test ! -e "$desktopfile" && cp org.kde.kaffeine.desktop ~/.local/share/applications/
...
BINSAVED="bash bwrap env sh tr tty kaffeine" # Enter here keywords to find and save in /usr/bin
...
function _remove_more_bloatwares() {
	...
	rm -rf ./"$APP".AppDir/.junest/usr/share/gtk-doc
    rm -rf ./"$APP".AppDir/.junest/usr/share/info
    rm -rf ./"$APP".AppDir/.junest/usr/share/licenses
    rm -rf ./"$APP".AppDir/.junest/usr/share/man
    rm -rf ./"$APP".AppDir/.junest/lib64/cmake
    rm -rf ./"$APP".AppDir/.junest/etc/pacman.d
    rm -rf ./"$APP".AppDir/.junest/etc/systemd
    rm -rf ./"$APP".AppDir/.junest/usr/share/pkgconfig
    rm -rf ./"$APP".AppDir/.junest/usr/share/doc
    rm -rf ./"$APP".AppDir/.junest/usr/lib/systemd
    rm -rf ./"$APP".AppDir/.junest/usr/lib/tmpfiles.d
    rm -rf ./"$APP".AppDir/.junest/usr/lib/girepository-1.0
    rm -rf ./"$APP".AppDir/.junest/usr/lib/pkgconfig
    rm -rf ./"$APP".AppDir/.junest/usr/lib/python3.12
    rm -rf ./"$APP".AppDir/.junest/usr/lib/sysctl.d
}
...
$HERE/.local/share/junest/bin/junest -n -b "$BINDS" -- bash -c "/usr/lib/vlc/vlc-cache-gen /usr/lib/vlc/plugins"
cd ..

rm depdeps*

ARCH=x86_64 ./appimagetool --comp zstd --mksquashfs-opt -Xcompression-level --mksquashfs-opt 20 ./$APP.AppDir
mv ./Kaffeine-x86_64.AppImage ./Kaffeine-$VERSION-x86_64.AppImage
